...
namespace Toodeloo
{
		...
		: _heightmap(filename + ".png")
	{
		...
		for(int y = 0; y < _heightmap.getHeight() - 1; y++)
		{
			...
			{
				std::vector<Graphics::Vertex> vertices(this->at(x, y));
	...
	float Heightmap::heightAt(int x, int y) const
	{
		uint8_t alpha = (_heightmap.at(x, y) & 0xff000000) >> 24;
		return alpha / 256.0;
	}

	std::vector<Toodeloo::Graphics::Vertex> Heightmap::at(int x, int y)
	{
